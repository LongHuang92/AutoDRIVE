...
def bridge(sid, data):
    if data:
        ...
        x, y, _ = f1tenth_1.front_camera_image.shape
        for detection in f1tenth_1.detections:
            ...
            print(f'Detected {class_name} with confidence {conf:.2f} at [{x1}, {y1}, {x2}, {y2}], relative position {left} to {right}')
            if (class_name == "car"):
                # TODO: LIDAR
                a=0
        ########################################################################
        ...
        if abs(steering_angle) > straights_steering_angle:
            ...
            target_speed = fast_speed
            prev_angle = steering_angle
        f1tenth_1.throttle_command = target_speed / 5 # [-1, 1]
        f1tenth_1.steering_command = np.clip(steering_angle, -max_steer, max_steer) # [-1, 1]

        ########################################################################

        json_msg = f1tenth_1.generate_commands(verbose=False) # Generate vehicle 1 message
